    ...
    override fun onReceive(context: Context, intent: Intent?) {
        ...
        if (FirebaseAuth.getInstance().currentUser != null) {
            ...
            if (lastDate != SimpleDateFormat("yyyyMMdd", Locale.ENGLISH).format(Date()).toInt()) {
                var word = repository.getWord(idUser, levelUser, repository.getLanguageUser(idUser)!!)
                var builder1: NotificationCompat.Builder? = null
                if (word.wordId == 0) {
                    repository.setLevelLocalAndFirebase(idUser, levelUser + 1)
                    word = repository.getWord(idUser, levelUser + 1, repository.getLanguageUser(idUser)!!)
