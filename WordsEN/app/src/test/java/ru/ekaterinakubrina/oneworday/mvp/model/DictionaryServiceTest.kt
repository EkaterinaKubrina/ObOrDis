...
class DictionaryServiceTest : TestCase() {
    ...
    private val dictionaryModel =
        DictionaryService(DictionaryDaoImpl(MyDbManager(ApplicationProvider.getApplicationContext())), wordsModel)
    ...
    @Test
    fun getWord() {
        val word = dictionaryModel.getWord("1", MyDbWordsEN.Level.LEVEL_A0, MyDbWordsEN.Language.ENGLISH)
        assertEquals(MyDbWordsEN.Status.NEW, word.status)
    ...
    fun addWordToUserTest() {
        ...
        val wordDto = dictionaryModelSpy.getWord("1", MyDbWordsEN.Level.LEVEL_A0, MyDbWordsEN.Language.ENGLISH)
    ...
    fun addUsersWordTest() {
        assertEquals(dictionaryModelSpy.getCountStudiedWord("1", MyDbWordsEN.Language.ENGLISH), 0)

        val wordDto = WordDto(0, "cat", "кот", "transcription", MyDbWordsEN.Language.ENGLISH)
        dictionaryModelSpy.addUsersWord(wordDto, 1, "1")

        assertEquals(1, dictionaryModelSpy.getCountStudiedWord("1", MyDbWordsEN.Language.ENGLISH))
    ...
    fun getLastDateAddedWordTest() {
        val word = dictionaryModelSpy.getWord("1", MyDbWordsEN.Level.LEVEL_A0, MyDbWordsEN.Language.ENGLISH)
        dictionaryModelSpy.addWordToUser("1", word)

        assertEquals(
            SimpleDateFormat("yyyyMMdd", Locale.ENGLISH).format(Date()).toInt(),
            dictionaryModelSpy.getLastDateAddedWord("1", MyDbWordsEN.Language.ENGLISH)
        )
    ...
    fun setStatusWordTest() {
        ...
        dictionaryModelSpy.setStatusWord("1", word.wordId, MyDbWordsEN.Status.STUDIED,  MyDbWordsEN.Status.NEW)
    ...
    fun getCountStudiedWordTest() {
        ...
        assertEquals(0, dictionaryModelSpy.getCountStudiedWord("1", MyDbWordsEN.Language.ENGLISH))
    }
